...
def build_kapso_system_prompt(
    ...
    sections = [
        ...
        (
            ...
            "\n"
            "## 🚫 POLÍTICA DE CANCELACIÓN (OBLIGATORIA)\n"
            "Si el contacto dice que NO puede asistir, NO llegará, quiere cancelar, "
            "le surgió un problema, ya no necesita la cita, etc:\n"
            "  PASO 1 — INVOCAR LA TOOL `cancelar_cita` ANTES de cualquier respuesta.\n"
            "  PASO 2 — Esperar el resultado de la tool.\n"
            "  PASO 3 — Solo DESPUÉS de que la tool confirme éxito, responder al contacto.\n"
            "  Si la tool devuelve error, decirle al contacto que hubo un problema y "
            "  pedirle que confirme nuevamente; NO afirmes que se canceló si la tool falló.\n"
            "PROHIBIDO escribir 'cancelo tu cita', 'queda cancelada', 'listo te la cancelo' "
            "o frases similares SIN haber invocado primero la tool. Eso deja al contacto con "
            "una cita zombi en el calendario del asesor.\n"
            "Si el contacto pide cancelar Y reagendar a la vez, el orden correcto es: "
            "primero `cancelar_cita`, después ofrecer disponibilidad y crear la nueva con `agendar_cita`."
        ),
        (
