...
import { Sparkles, Send, Loader2, X, CheckSquare, Flag, Calendar, Search } from 'lucide-react';
...
interface TaskSearchCreateProps {
...
export const TaskSearchCreate: React.FC<TaskSearchCreateProps> = ({
  onCreateTask,
  onSearch,
  searchQuery,
  projectId,
  ...
  // Auto-resize textarea in create mode
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    } else if (mode === 'create' && textareaRef.current) {
  ...
  const handleGenerate = async () => {
    ...
    // Debug: verify userContext
    console.log('[TaskSearchCreate] userContext:', userContext);
    console.log('[TaskSearchCreate] authUid:', userContext?.authUid);

    if (!userContext?.authUid) {
      setError('Usuario no autenticado. Por favor recarga la página.');
      return;
    }
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (mode === 'create' && e.key === 'Enter' && !e.shiftKey && !generatedTask) {
      e.preventDefault();
      handleGenerate();
    }
  };
  ...
  return (
    <div className={cn("w-full", className)}>
      {/* Generated Task Preview (only in create mode) */}
      {mode === 'create' && generatedTask && (
          ...
          {mode === 'search' ? (
            ...
            <>
              <textarea
                ...
                placeholder="Describele tu tarea a Monica..."
                rows={1}
                disabled={isGenerating || !!generatedTask || disabled}
              ...
              <button
                ...
                disabled={!text.trim() || text.trim().length < 3 || isGenerating || !!generatedTask || disabled}
                className={cn(
                  "p-2 rounded-lg transition-all flex-shrink-0",
                  text.trim().length >= 3 && !generatedTask && !disabled
