# 🤖 Manual del Agente: Protocolo de UI Dinámica v5
...
### 11. Task Board - Gestión de Tareas v5
Para crear, editar, eliminar y asignar tareas con CRUD completo.
...
```json:ui
{
  ...
  "data": {
    "tasks": [
      {
        "id": "1",
        "title": "Revisar diseño del dashboard",
        "description": "Validar colores y tipografía con el equipo",
        "status": "in_progress",
        "priority": "high",
        "assignee": "Ana García",
        "dueDate": "2025-01-15",
    ...
    "assignees": ["Ana García", "Carlos Ruiz", "María López", "Juan Pérez"],
    "view": "list",
    "actions": [
      { "id": "add_task", "label": "Nueva Tarea", "icon": "Plus", "variant": "primary" },
      { "id": "export_tasks", "label": "Exportar", "icon": "Download", "variant": "secondary" }
    ]
  }
}
```
...
- `assignee` (string, opcional) — Nombre del asignado
- `dueDate` (string ISO, opcional) — Fecha límite
- `tags` (string[], opcional) — Etiquetas

**Interacciones emitidas:**
- `TASK_ADD` — Al crear una tarea nueva
- `TASK_EDIT` — Al editar una tarea existente
- `TASK_DELETE` — Al eliminar una tarea
- `TASK_STATUS_CHANGE` — Al cambiar el estado
- `TASK_ASSIGN` — Al reasignar una tarea

---

### 12. Grid - Cuadrícula de Tarjetas v5
```json:ui
{
  "type": "grid",
...
| `task_board` | `primary` o `info` | `tasks[]`, `assignees[]`, `view` |
